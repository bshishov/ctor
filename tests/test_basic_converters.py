...
def test_primitive_string_converter_load_as_is(s):
    converter_load_produces_same_result(
        converter=PrimitiveTypeConverter(str),
        value=s
    )
...
def test_list_converter_load(value):
    converter_load_produces_same_result(
        converter=ListConverter(ExactConverter()),
        value=value
    )
...
def test_list_converter_raises_if_item_converter_raises():
    converter_load_raises_load_error(
        converter=ListConverter(PrimitiveTypeConverter(int)),
        value=[1, 'INVALID', 3]
    )
...
def test_dict_converter_load(value):
    converter_load_produces_same_result(
        converter=DictConverter(ExactConverter()),
        value=value
    )
...
def test_set_converter_raises_if_item_converter_raises():
    converter_load_raises_load_error(
        converter=SetConverter(PrimitiveTypeConverter(int)),
        value={1, 'not an int', 2}
    )
...
def test_union_converter_load(value, options: JsonSerializationContext):
...
@pytest.mark.parametrize('value', [0, 1, True, False])
def test_union_converter_dump(value, options: JsonSerializationContext):
    converter = UnionTypeConverter(
        PrimitiveTypeConverter(int),
        PrimitiveTypeConverter(bool),
    )
...
def test_union_converter_load_first_suitable(value, expected, options: JsonSerializationContext):
    converter = UnionTypeConverter(
...
def test_union_converter_not_raises_if_no_converter_matches_in_first_nested_union(value, options: JsonSerializationContext):
    converter = UnionTypeConverter(
        UnionTypeConverter(
            PrimitiveTypeConverter(int)
        ),
...
def test_datetime_isoconverter_load(options: JsonSerializationContext):
    # TODO: нужно проверить timezone с другими сервисами интегрированными
...
@pytest.mark.parametrize('enum_type,value,expected', [
...
def test_enum_converter_LoadError_if_invalid_value(options: JsonSerializationContext, enum_type):
